...
public class DocWordMatrixGenerator {
	...
	public static void main(String args[]) throws SQLException {
		...
		DocWordMatrixGenerator generator = new DocWordMatrixGenerator(args[0]);
		generator.generate();
	...
	private void setArticles() throws SQLException {
		...
		ResultSet rs = db.executeQuery("SELECT * FROM article WHERE date(timestamp) >= date(subdate(now(), INTERVAL 10 DAY)) and date(timestamp) <= date(now()) ORDER BY timestamp");
		while(rs.next()) {
	...
	private void setUserArticles() throws SQLException {
		...
		ResultSet rs = db.executeQuery("SELECT * FROM user_article WHERE user_id='" + userId + "' AND date(timestamp) >= date(subdate(now(), INTERVAL 10 DAY)) and date(timestamp) <= date(now()) ORDER BY timestamp");
		while(rs.next()) {
